...
fields_to_read=["limitedTopics","parsed_text"]
    ...
    if prediction!=None:
        ...
        for k in prediction:
            ...
            if k not in prediction_map:
                prediction_map[k]={"total":0,"wordcount":dict()}

            prediction_map[k]["total"]+=1
            
            for word in text:
                
                if word not in prediction_map[k]["wordcount"]:
                    prediction_map[k]["wordcount"][word]=0
                
                prediction_map[k]["wordcount"][word]+=1
